...
# ----------------------------------------------------------------------------
from T1c_data_prep1 import load_test_data
...
def dice_score_full(y_pred, y_true):
    ...
    for i in range(len(PredictedClasses['class'].values)):
        ...
        print("Dice =",DSC[classNumber])
        print("PPV =",PPV[classNumber])
        print("Sensitivity= ",Sensitivity[classNumber])
        avgdsc[classNumber] += DSC[classNumber]
        avgppv[classNumber] += PPV[classNumber]
        avgsen[classNumber] += Sensitivity[classNumber]
        #print (avgdsc[classNumber],"**",avgppv[classNumber],"**",avgsen[classNumber])
...
def load_trained_agu_model():   
    ...
    json_file = open('NewUnetBceNormT1c/cnn_BRATs_unet_new.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model    
    loaded_model.load_weights("NewUnetBceNormT1c/cnn_BRATs_unet_new.h5")
...
def show_img_32(imgs_seg1, imgs_seg2):#, imgs_seg3):
    ...
    patient_HGG1_5 = [21]
    first_slice_in_each_patient_HGG1_5 = [44]
    patient_HGG11_5 = [37]
    first_slice_in_each_patient_HGG11_5 = [67]
    patient_HGG11_3 = [22]
    ...
    patients = patient_HGG11_5
    first_slice_in_each_patient = first_slice_in_each_patient_HGG11_5
    slice_number = 0
    counter = 0
    index = 0
    patient_number = 1
...
def show_2Dimg_3(img_1, img_2, patient_number, slice_number):
    ...
    #plt.imshow(img, zorder=0, extent=[0.5, 8.0, 1.0, 7.0])
    fig.savefig('/home/res-safia-fatima/T1c/NewUnetBceNormT1c/HGG11_5/'+'Patient'+str(patient_number)+'Slice'+str(slice_number)+'.png')   
# save the figure to file
    plt.close(fig) 
    #ax[2].imshow(img_3, cmap=plt.cm.gray)1
    #plt.show()
        
def cvt2color_img(img_src,patient_number,slice_number):
    imgddd = mpimg.imread('/home/res-safia-fatima/T1c/TestingT1c/HGG11_5/threshold_patient_HGG11_5/'+'Patient'+str(patient_number)+'Slice'+str(slice_number)+'.png')
