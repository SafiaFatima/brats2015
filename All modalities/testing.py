...
# ----------------------------------------------------------------------------
from data_prep_noaug import load_test_data
...
def load_trained_agu_model():   
    ...
    json_file = open('cnn_BRATs_unet_new.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model    
    loaded_model.load_weights("cnn_BRATs_unet_new.h5")
...
def report(y_true,y_pred):
    ...
    for n in range(y_true.shape[0]):
        #print (len(y_true[n][:]))
        print(classification_report(y_true[n][:], y_pred[n][:]))
...
def show_img_32(imgs_seg1, imgs_seg2):#, imgs_seg3):
    #these are the starting and ending slices of each test patient who passed the thurshold 
    #it is used for localiation of each slice of test patient
    patient_HGG1_1=[28]
    ...
    patients = patient_HGG1_2
    first_slice_in_each_patient = first_slice_in_each_patient_HGG1_2
    slice_number = 0
    counter = 0
    index = 0
    patient_number = 1
...
def show_2Dimg_3(img_1, img_2, patient_number, slice_number):
    ...
    fig.savefig('/home/res-safia-fatima/code/NewUnetBceNorm/HGG1_2/'+'Patient'+str(patient_number)+'Slice'+str(slice_number)+'.png')   
# save the figure to file
    plt.close(fig) 
        
def cvt2color_img(img_src,patient_number,slice_number):
    imgddd = mpimg.imread('/home/res-safia-fatima/code/Testing/HGG1_2/threshold_patient_HGG1_2/'+'Patient'+str(patient_number)+'Slice'+str(slice_number)+'.png')
...
def test_network_aug():
    ...
    print('Imgs test label shape', len(imgs_label_test.shape))
    ...
    #print(len(imgs_label_test.shape))
